...
Quatre départements forestiers réels (Landes 40, Lozère 48, Creuse 23, Vosges 88),
des communes et des zones Natura 2000 qui recouvrent effectivement les coupes.

Comptes (les bénévoles partagent le mot de passe ``volunteer``) :

- ``admin@example.com`` / ``admin`` — administrateur, les quatre départements
- ``volunteer@example.com`` — Alice, Landes
- ``bruno@example.com`` — Bruno, Lozère
- ``chloe@example.com`` — Chloé, Creuse et Vosges
- ``david@example.com`` — David, compte désactivé (Vosges)

Quatorze signalements couvrent chaque statut, chaque profil forestier, des cas
mono et multi-coupes, les seuils des règles (surface, pente, zonage) de part et
d'autre, une demande d'attribution en attente et un étalement des dates de coupe.
Trois formulaires : deux complets (Mende, La Bresse) et un brouillon (Labouheyre).
"""
...
def cut(
    ...
    *,
    ...
    zonings: list[EcologicalZoning] | None = None,
    dx: float = 0.0,
    dy: float = 0.0,
) -> ClearCut:
