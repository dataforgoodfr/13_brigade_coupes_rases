...
def is_in_local(download_path, **kwargs):
    if os.path.exists("dags/"+download_path):
        if "mosaics_tropisco_warnings_france_date.tif" in os.listdir("dags/" + download_path):
            result = True
    else:
        result = False    
    
    kwargs['ti'].xcom_push(key='is_in_local', value=result)
    # kwargs['ti'].xcom_push(key='variss', value=my_result)
...
def polygonize_tif(**kwargs): 
    raster_path = kwargs['ti'].xcom_pull(task_ids='regroup_sufosat_days', key='regrouped_tif_path')
...
def process_geo_data(**kwargs):
    vector_path = kwargs['ti'].xcom_pull(task_ids='polygonize_tif', key='vector_path')
