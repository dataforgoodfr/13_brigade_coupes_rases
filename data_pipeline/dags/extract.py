...
def check_tif_in_s3(bucket_name, s3_key_prefix, s3_hook, my_filename, **kwargs):
    in_s3 = False
    ...
    kwargs["ti"].xcom_push(key="in_s3", value=in_s3)
...
def update_metadata(bucket_name, s3_key, s3_hook, **kwargs):
    metadata = kwargs["ti"].xcom_pull(task_ids="extract_pipeline.is_metadata_update", key="metadata")
    my_metadata = kwargs["ti"].xcom_pull(task_ids="extract_pipeline.is_metadata_update", key="mymetadata")

    if not my_metadata or "version" not in my_metadata:
        version = 1.0
    else:
        version = my_metadata["version"] + 0.1
    ...
    kwargs["ti"].xcom_push(key="update_meta_data", value=meta_data_json)
