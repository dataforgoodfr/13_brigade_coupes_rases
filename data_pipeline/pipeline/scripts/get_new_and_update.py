...
def split_new_and_updated_clusters(gdf_new, gdf_ref, distance_threshold=50):
    ...
    joined = gpd.sjoin(gdf_new, gdf_ref_buffered, how="left", predicate="intersects")
    ...
    # Sauvegarde
    gdf_updated.to_file(
        DATA_DIR / "sufosat" / "clusters_updated.fgb", driver="FlatGeobuf"
    )
    gdf_truly_new.to_file(
        DATA_DIR / "sufosat" / "clusters_new.fgb", driver="FlatGeobuf"
    )
...
def update_geometries(distance_threshold=50):
    ...
    gdf_ref = gpd.read_file(
        str(DATA_DIR / "sufosat_reference" / "sufosat_clusters_enriched.fgb")
    )
    ...
    joined = gpd.sjoin(
        gdf_updated_copy, gdf_ref_buffered, how="left", predicate="intersects"
    )
    ...
    print("Mise à jour des géométries:")
    ...
    # Sauvegarde
    gdf_ref_updated.to_file(
        DATA_DIR / "sufosat_reference" / "sufosat_clusters_enriched_UPDATED.fgb",
        driver="FlatGeobuf",
    )
    ...
    print("\n Fichiers sauvegardés:")
    print(
        f"   - {DATA_DIR / 'sufosat_reference' / 'sufosat_clusters_enriched_UPDATED.fgb'}"
    )
