...
def load_processed_vector_data():
    ...
    if not os.path.exists(filepath):
        raise FileNotFoundError(
            f"Le fichier {filepath} n'existe pas. Veuillez exécuter 'process_vector_data' d'abord."
        )
...
def clean_clusters(clear_cut_cluster=None, save_filepath=None):
    ...
    if clear_cut_cluster is None:
        filepath = os.path.join(
            configs["transform_sufosat"]["download_path"], "spatial_joined_data.geoparquet"
        )
        if not os.path.exists(filepath):
            raise FileNotFoundError(
                f"Le fichier {filepath} n'existe pas. Veuillez exécuter 'spatial_join' d'abord."
            )
...
def group_cuts(clear_cut_cluster=None, clear_cut=None):
    ...
    if clear_cut_cluster is None or clear_cut is None:
        filepath = os.path.join(
            configs["transform_sufosat"]["download_path"], "cleaned_clusters.geoparquet"
        )
        if not os.path.exists(filepath):
            raise FileNotFoundError(
                f"Le fichier {filepath} n'existe pas. Veuillez exécuter 'clean_clusters' d'abord."
            )
        clear_cut_cluster = read_parquet(filepath)
...
def calculate_areas(clear_cut_group=None):
    ...
    if clear_cut_group is None:
        ...
        if not os.path.exists(filepath):
            raise FileNotFoundError(
                f"Le fichier {filepath} n'existe pas. Veuillez exécuter 'group_cuts' d'abord."
            )
...
def calculate_concave_hulls(clear_cut_group=None):
    ...
    if clear_cut_group is None:
        ...
        if not os.path.exists(filepath):
            raise FileNotFoundError(
                f"Le fichier {filepath} n'existe pas. Veuillez exécuter 'calculate_areas' d'abord."
            )
        clear_cut_group = read_geoparquet(filepath)
...
def export_results(clear_cut_group=None):
    ...
    if clear_cut_group is None:
        filepath = os.path.join(
            configs["transform_sufosat"]["download_path"], "concave_hulls.geoparquet"
        )
        if not os.path.exists(filepath):
            raise FileNotFoundError(
                f"Le fichier {filepath} n'existe pas. Veuillez exécuter 'calculate_concave_hulls' d'abord."
            )
        clear_cut_group = read_geoparquet(filepath)
...
def run_pipeline():
    try:
        ...
        save_data(
            ...
            os.path.join(configs["transform_sufosat"]["download_path"], "cut_areas.geoparquet"),
        )
