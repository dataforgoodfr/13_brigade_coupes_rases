...
class DatabaseSeeder:
    ...
    def update_serial(self, table: str) -> None:
        ...
        with self.engine.connect() as conn:
            ...
            # After update
            current_val = conn.execute(text(f"SELECT currval('{table}_id_seq')")).scalar()
    ...
    def wipe_database(self) -> None:
        ...
        with self.engine.connect() as conn:
            ...
            truncate_stmt = f"TRUNCATE TABLE {', '.join(self.TABLES)} RESTART IDENTITY CASCADE"
    ...
    def seed_ecological_zonings(self, natura2000_concat_filepath: Path) -> None:
        ...
        natura2000_concat = load_gdf(natura2000_concat_filepath).drop(columns="geometry")
    ...
    def add_city_id_to_sufosat(self, sufosat: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
        ...
        city_ids = pd.read_sql(
            "SELECT id AS city_id, zip_code AS city_insee_code FROM cities", con=self.engine
        )
        display_df(city_ids)
        ...
        logging.info(f"Merging with city data ({length_before_merge} records before merge)")
        ...
        assert (
            sufosat["city_id"].isna().sum() == 0 and len(sufosat) >= length_before_merge - 100
